...
async function stickerCommand(sock, chatId, message) {
    ...
    if (!mediaMessage) {
        ...
        await sendError(sock, chatId, messageToQuote, 'missingMedia', "🖼️ *No Media Found*\n\nI need an image or video to create a sticker! 📸\n\n*How to use:*\n• Reply to an image/video with .sticker\n• Or send an image/video with .sticker as caption\n\n*Supported formats:* Images & Videos");
        return;
    }
    ...
    try {
        ...
        if (!mediaBuffer) {
            await sock.sendMessage(chatId, { 
                text: '📥 *Media Download Failed*\n\nI couldn\'t download the media file. Make sure it\'s a valid image or video! 📸\n\n*Try with a different media file.*\n\n*Powered by EliTechWiz*',
                contextInfo: {
                    forwardingScore: 999,
                    isForwarded: true,
        ...
        const json = {
            ...
            'sticker-pack-name': settings.packname || 'EliTechWiz',
            'emojis': ['🤖']
        };
        ...
        await handleError(sock, chatId, messageToQuote, error, 'processing');
    }
}
